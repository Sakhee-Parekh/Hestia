...
app.route('/sign_in')
    .get(function (req, res){
        if(req.session.logged_in){
            console.log("Attempt to go to login page while already logged in, current user: " + request.session.username);
    ...
    .post(async function(req,res){
        ...
        User.find({"username": newUser.username}).lean().then(item=>{
            if(item.length>0){
                ...
                User.find({"email": newUser.email}).lean().then(item=>{
                    if(item.length>0){
                        console.log("email is already registered")
                        res.redirect("/sign_up?error=2")
                    }
                    else{
                        newUser.save().then(console.log("New user created")).catch(err=> console.log("Error when creating user:", err));
